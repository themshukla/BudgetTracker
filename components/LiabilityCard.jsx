...
const LiabilityCard = ({
  ...
  const longTermSwipeableRefs = useRef([]);
  ...
  useEffect(() => {
    ...
    const longTerm = card?.items?.longTerm ?? [];
    setHasContent(normal.length > 0 || longTerm.length > 0);
  }, [card.items]);
  ...
  const routes = [
    { key: "normal", title: "Liabilities" },
    { key: "longTerm", title: "Longterm" },
  ];
